...
// O(log_10 (n))
int Integer::digits(long long x)
{
    if(x == 0) return 1;
    int c = 0;
    while(x != 0)
...
Integer Integer::partHigh(int index) const
{
    Integer toR;
...
Integer Integer::lowPart(int index) const
{
    Integer toR;
...
Integer Integer::conventionalForm(const Integer& num1, const Integer& num2)
{
    Integer product;
...
Integer Integer::karatsuba(const Integer& num1, const Integer& num2)
{
...
Integer Integer::operator*(const Integer& other) const
{
    ...
    // std::cout << "In integer mult: " << *this << " * " << other << "\n";
...
std::ostream& operator<<(std::ostream& os, const Integer& number)
{
    ...
    {
        for(int j = 0; j < Integer::digitsByBlock - Integer::digits(number.digitAt(i)); j++)
            os << '0';
        os << number.digitAt(i);
    }  
...
List<int> Integer::getList() const
{
...
Integer Integer::recursiveDivision(const Integer& num1, const Integer& num2)
{
    if(num1.numberSize() < 15 || num2.numberSize() < 15)
        return num1/num2;
