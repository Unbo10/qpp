...
public:
    ...
    Polynomial to_integer_poly() {
        ...
        for(int i = 0; i < n; i++) {
            ...
            Integer gcd = Integer::binaryEcludian(lcm_den, current_den);
    ...
    Integer find_gcd_of_poly_terms() {
        ...
        Integer gcd;
        gcd = Integer::binaryEcludian(sparse[0].coeff.numerador, sparse[1].coeff.numerador);
        ...
        for(int i = 2; i < (int)sparse.size(); i++) {
            // std::cout << "GCD: " << gcd << "\n";
            gcd = Integer::binaryEcludian(gcd, sparse[i].coeff.numerador);
        }
    ...
    Integer find_lcm_of_poly_terms() {
        ...
        for(int i = 1; i < (int)sparse.size(); i++) {
            Integer current_den = sparse[i].coeff.denominador;
            Integer gcd = Integer::binaryEcludian(lcm, current_den);
    ...
    Polynomial operator/(const Polynomial& other) const {
        ...
        r = u;
        Polynomial q;
        q.dense.resize(u.degree - other.degree + 1, Rational(0));
